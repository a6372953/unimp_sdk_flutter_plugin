...
//                                    self.uniMPInstance = instance
            ...
            if(!isFileExistsInDocuments(filename: appid+".wgt")){
                ...
                downloadFileAndSaveToDocuments(url: URL(string: url)!){ [weak self] in
                    self?.installAndOpenUniMP(appid: appid, url: url)
                }
            }
            installAndOpenUniMP(appid: appid, url: url)
        }
    }
    
    func installAndOpenUniMP(appid: String, url: String){
        let docsDir = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)[0]
        let filePath = docsDir + "/" + appid + ".wgt"
        print(filePath)
        do{try
        DCUniMPSDKEngine.installUniMPResource(withAppid: appid, resourceFilePath: filePath, password: nil)
        }catch{
